"""
Generate Height vs. Timestamp plots for every processed forklift CSV.

The script scans ``real_data_with_headers`` (or a user-provided directory),
looks for ``*_forklift.csv`` files, and writes one PNG per file into the
``plots`` directory. Use it after running ``assign_columns.py`` so the inputs
already carry headers and forklift/truck labels.
"""
...
DEFAULT_PLOTS_DIR = Path(__file__).parent / "plots" / "forklift_heights"


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot height changes over time for each forklift CSV."
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
